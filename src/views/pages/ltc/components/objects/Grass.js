...
export default function Grass({ options = { bW: 0.12, bH: 1, joints: 5 }, width = 200, instances = 50000 }) {
  ...
  const groundGeo = useMemo(() => {
    const geo = new Geometry().fromBufferGeometry(new THREE.PlaneGeometry(width, width, 200, 200))
    geo.verticesNeedUpdate = false
    geo.lookAt(new THREE.Vector3(0, 1, 0))

    
    console.log("aki", geo.vertices.length)
    for (let i = 0; i < geo.vertices.length; i++) {
        const v = geo.vertices[i]
        v.y = getYPosition(v.x, v.z, coord, i)
      }
  ...
  const [groundGeo2,setgroundGeo2] = useState({});

  React.useEffect(() => {
    fetch("/resources/coord2.txt")
      .then((r) => r.text())
      .then((text) => {

        let res = text.split('\t').map(value => {return parseFloat(value)});
        //setCoord(res);//listToMatrix(res, 200))
        
        const geo = new Geometry().fromBufferGeometry(new THREE.PlaneGeometry(401, 500, 200, 200))
    geo.verticesNeedUpdate = false
    geo.lookAt(new THREE.Vector3(0, 1, 0))
    ...
    for (let i = 0; i < geo.vertices.length; i++) {
      const v = geo.vertices[i];
      v.y = getYPosition(v.x, v.z, res, i);
    }
    
    geo.computeVertexNormals()
    setgroundGeo2(geo.toBufferGeometry());
...
function getYPosition(x, z, coord, index) {
